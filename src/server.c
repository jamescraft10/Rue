...
char *path_to_code(char *path1) {
    if(path1[0] == '/' && path1[1] == 'a' && path1[2] == 'p' && path1[3] == 'i') {
        return rue(path1);
    }
...
char *path_to_type(char* path) {
    if(strcmp(&path[strlen(path)-5], ".html") == 0 || strcmp(&path[strlen(path)-4], ".htm") == 0) {
        return "text/html";
...
char *path_to_res_code(char *path) {
    if(strcmp(&path[strlen(path)-4], ".png") == 0 || strcmp(&path[strlen(path)-4], ".jpg") == 0 || strcmp(&path[strlen(path)-5], ".jpeg") == 0) {
        return "304 Not Modified";
    } else if(strcmp(&path[strlen(path)-5], ".html") == 0 || strcmp(&path[strlen(path)-4], ".css") == 0 || strcmp(&path[strlen(path)-3], ".js") == 0 || strcmp(&path[strlen(path)-3], ".ts") == 0 || strcmp(path, "/") == 0) {
        return "200 OK";
    }
...
int main(int argc, char *argv[]) {
    ...
    while(1) {
        ...
        // response
        response res;
        res.http_code = path_to_res_code(req.path); // put all that uses req.path before path_to_code() because that changes the req.path value
